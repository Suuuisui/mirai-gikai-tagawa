...
type BillContent = Database["public"]["Tables"]["bill_contents"]["Row"];
...
/**
 * 否決・不認定など「異例の議決」かどうかを判定する。
 * スコア加点のほか、adminのトップページ編集画面のバッジ表示にも使う。
 */
export function isControversialStatus(statusNote: string | null): boolean {
  return statusNote !== null && CONTROVERSIAL_STATUS_PATTERN.test(statusNote);
}
...
export function computeBillInterestScore(
  ...
  if (isControversialStatus(bill.status_note)) {
...
export function isHotTopicBill(
  ...
  return isHotTopicScore(
    computeBillInterestScore(bill, now),
    bill.submitted_date,
    now
  );
}
...
/**
 * `isHotTopicBill` の判定を、計算済みの興味度スコアから行うバリアント。
 * adminのトップページ編集画面のように、スコアを別途表示するため既に
 * 計算している呼び出し元が、本文の再スキャンなしで判定できるようにする。
 */
export function isHotTopicScore(
  score: number,
  ...
  return score > MIN_NOTABLE_SCORE;
}
