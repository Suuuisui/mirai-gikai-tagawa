...
- `Blocked` -> エージェントにとっての終了状態。実行が真の環境／セットアップブロッカー（必要なツール・認証・権限の欠如、エージェント側で解決できない設定不一致）に詰まった状態。workpad には何が欠けているか・人間が必要な正確なアクションが記録される。変更しない。人間がブロッカーを解消し、`Todo` に戻すと再開する。
...
3. 一致するフローにルーティングする:
   ...
   - `Blocked` -> 何もせずシャットダウンする（人間がアンブロックするまでエージェントの終了状態）。
...
- 状態が終了状態（`Blocked`、`QA`、`Done`、または他の設定された終了状態）の場合、何もせずシャットダウンする。
